...
force_eclipse_watcher()
{
	while(1)
	{
		...
		back_to_the_eclipse(); //Changes made June 12th 2020.
		reveal_meteor();	     //Changes made June 12th 2020.
		...
		back_to_the_future();   //Changes made June 12th 2020.
		hide_meteor();       //Changes made June 12th 2020.
...
//	level endon("sidequest_sq_complete");
	...
	while(1)
	{
		while(level._sundial_buttons_pressed < 1)	// Wait for all 4 buttons to be pressed. //Changes made June 13th 2020.
		{
			wait(0.1);
		}
...
sundial_button_already_pressed_by(who, buttons)
{
	...
	for(i = 0; i < buttons.size; i ++)
	{
		...
		{
			return true; //Let's 1 player to hit all the 4 triggers in the spawn room. Value previously = true, now false; //Changes made June 13th 2020.
		}
	}
...
//	level endon("sidequest_sq_complete");
	...
	{
		self.trigger Delete(); //Commenting this out will result that the player is not able to press the sun dial buttons at all.
...
//	self.trigger trigger_off();
	...
	self.trigger.radius = 48; //Originally set to 48 
	...
	while(1)
	{
		...
		{
			self.triggering_player = who; //This needs to be changed from who to undefined

			level._sundial_buttons_pressed ++; //This needs to be changed from ++ to --
