...
monitor_player_zipline(zipline_vehicles)
{
	...
	while(1)
	{
		...
		{
			continue;  //changing this to break will not let the player get on the zipline!!
		}
		...
		{
			who.is_ziplining = true;  //Changing this to false doesn't seem to have no effect... :o ,Otto,
...
is_ok_to_zipline(zip_path)
{
	...
	if( self getstance() == "stand" || is_true(self.divetoprone))
	{
		...
		{
			return true; //Try true instead of false once set to true, player can run into the zipline but only once... Look more into it later..! ,Otto,
		}		
