...
main()
{
	level.player_too_many_weapons_monitor = false; //Changed to false July 6, 2020!
	...
	level.disable_player_damage_knockback = false;
...
post_all_players_connected()
{
	...
	level thread shangri_la_dev_script(); //Changes made for Shang's EE !!Must thread before loading to shang!
...
init_dvars()
{
	...
	setSavedDvar( "r_skyTransition", "" );	          //Changes made June 15th 2020
	...
	if ( level.script == "zombie_temple" )       //Changes made June 16th so that the sky is set to night time if level == shangri la
	{
...
onPlayerConnect_clientDvars()
{
	self SetClientDvars( "cg_deadChatWithDead", "1",
		...
		"hud_showStance", "0",
		"cg_thirdPerson", "0",
		//"cg_debugposition", "1", //Try to see if you can see player coordinates on the screen 11.04.2020
		"cg_fov", "95", //Changes the fov to 95 when player connects to a match!
		"cg_thirdPersonAngle", "0",
		"ammoCounterHide", "1",
		"miniscoreboardhide", "1",
		//"sv_EnableDevCheats", "1", // Try enabling cheats to zombiemode 11.04.2020.
		"cg_drawCrossHair", "1", // Try disabling crosshair 11.04.2020 //Set Value to 0 if wanting the effect.
		"cg_drawSpectatorMessages", "0",
		"ui_hud_hardcore", "0", //Try changing it to 1 and see what happens as of 11.04.2020 //Changing it to 1 did nothing.
		"playerPushAmount", "1" );
	...
	if( level.script == "zombie_temple" ) //Changes made on June 16th 2020. If set level = shangri la, enable developer script to complete the ee with any amount of players.
	{
		self endon ( "disconnect" );
	...
	{
		self endon ( "disconnect" );
		self setClientDvar( "developer_script", "0" );
	}
	...
	if( level.script == "zombie_moon" )
	{
		...
		self setClientDvar( "developer_script", "1" );
		 //Changes made august 2020/Ultimateman
	...
	/*
	if(!level.zombie_moon)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_theater)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_pentagon)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_cosmodrome)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_coast)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_cod5_prototype)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_cod5_sumpf)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_cod5_factory)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	if(!level.zombie_cod5_asylum)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
    */
    /*
	if(!level.zombie_temple)
	{
		self setClientDvar( "r_skyTransition", "0" );
	}
	else
...
#/
		...
		self SetClientDvars( "cg_thirdPerson", "0",
			"cg_fov", "95",
			"com_maxfps", "333",
			"sv_EnableDevCheats", "0", //Don't include in the patch set it to 0
			"sv_cheats", "0",  //Don't include in the patch set it to 0
			"developer", "0", //SET THIS TO 0 WHEN PUBLISHING THE WHOLE PATCH
			"cg_debugposition", "0" ); //Don't include in the patch set it to 0
			//"ammoCounterHide", "1",
			//"miniscoreboardhide", "1",
			//"cg_ScoresColor_TransparencyZombie", "0",
			//"cg_scoreboardtextoffset", "1000",
			//"cg_thirdPersonAngle", "0" );
		...
		if ( level.script == "zombie_coast" || "zombie_cosmodrome" )
		{
			self giveWeapon( "knife_ballistic_sickle_upgraded_zm" );
			self giveMaxAmmo( "knife_ballistic_sickle_upgraded_zm" );
		}
		else if ( level.script == "zombie_theater" || "zombie_pentagon" || "zombie_moon" || "zombie_temple" || "zombie_cod5_factory" || "zombie_cod5_sumpf" )
		{
			self giveWeapon( "knife_ballistic_bowie_upgraded_zm" );   //include in the public patch this way players can do bowie stuff faster
			self giveMaxAmmo( "knife_ballistic_bowie_upgraded_zm" ); //include in the public patch  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
		}										 
		//self.score = 1000; 


        //wait 4;
		//iprintln( "Black Ops Zombies Old School Glitches Created by ^1Ultimateman^7. ^2Enjoy :) " );
		...
		if( isdefined( self.initialized ) )
		{
			if( self.initialized == false )
			{
				...
				// set the credits for the patch on screen when players have spawned.
				self thread ultimateman_intro_credits();
		...
		/*
		wait 3.5;
		self iprintln( "Reading ^1Ultimateman's ^7custom ^1patch ^7data!" );
		wait 2.0;
		self iprintln( "Started ^3loading^7:" );
		wait 2.0;
		self iprintln( "^3zombie_theater_patch^7..." );
		wait 1.0;
		self iprintln( "^3zombie_pentagon_patch^7..." );
		wait 1.0;
		self iprintln( "^3zombie_cosmodrome_patch^7..." );
		wait 1.0;
		self iprintln( "^3zombie_coast_patch^7..." );
		wait 1.0;
		self iprintln( "^3zombie_temple_patch^7..." );
		wait 1.0;
		self iprintln( "^3zombie_moon_patch^7..." );
		wait 1.0;
		self iprintln( "^3zombie_cod5_patch^7..." );
		wait 1.0;
		self iprintln( "Loading ^2Successful^7!" );
		wait 5.5;
		self iPrintlnBold( "Welcome to my ^7Old School ^1Zombies ^7Glitching mode made by ^1Ultimateman!" );  //Welcome to my Old School Custom Patch made by Ultimateman
		*/
	}
}
...
take_additionalprimaryweapon()
{
	...
	if ( primary_weapons_that_can_be_taken.size >= 98 )
	{
...
set_third_person( value )
{
	...
	{
		self SetClientDvars( "cg_thirdPerson", "0",
			"cg_fov", "95", //Should be set to 95 instead of 70 because once player comes back to life he should be back at 95 instead of the default 70.
			"cg_thirdPersonAngle", "0" );
...
round_start()
{
	...
	for (i = 0; i < players.size; i++)
	{
		...
		players[i] SetClientDvars( "ammoCounterHide", "0", //needs to be set as zero before publishing the patch! 
				"miniscoreboardhide", "0" ); 			  //needs to be set as zero before publishing the patch!
...
player_exit_level()
{
	...
	{
		self.game_over_bg.foreground = false;
...
intermission()
{
	...
	for( i = 0; i < players.size; i++ )
	{
		...
		players[i] SetClientDvars( "cg_thirdPerson", "0",
			"cg_fov", "95", "developer_script", "0" );   // CHANGES June 16th 2020!
			                                            //This changes it back to 70 till you load a new zombies game=? //Otto //Changed to 90 all the time as of today June 16th 2020.
		                                               //Put developer_script 0 here!! If a certain map has developer script enabled (for solo EE purposes)
		                                              //this function will kill it and change it back to 0 until a map with dev script enabled is loaded.
...
ultimateman_intro_credits()
{
	wait 3.5;
    iprintln( "Reading ^1Ultimateman's ^7custom ^1patch ^7data!" );
    wait 2.0;
	iprintln( "Started ^1loading^7:" );
	wait 2.0;
	iprintln( "^2Custom Vision Sets^7..." );
	wait 1.0;
	iprintln( "^2Deathbarrier Script^7..." );
	wait 1.0;
	iprintln( "^2Collision Script^7..." );
	wait 1.0;
	iprintln( "^2Knockback Script^7..." );
	wait 1.0;
	iprintln( "^2Hud Overlay Script^7..." );
	wait 1.0;
	iprintln( "^2Custom Dev Binds^7..." );
	wait 1.0;
	iprintln( "Loading ^2Successful^7!" );
	wait 5.5;
	iPrintlnBold( "Welcome to my ^7Old School ^1Zombies ^7Glitching mode made by ^1Ultimateman!" );  //Welcome to my Old School Custom Patch made by Ultimateman
}

//DEV SCRIPT FOR SHANGRI LA, MADE BY ULTIMATEMAN!! 
shangri_la_dev_script()
{
	...
	for( ;; )
	{
		...
		else if ( level.script == "zombie_temple" && (self.intermission == "true" ) )
		{
			self setClientDvar( "developer_script", "0");
		}
	}
...
// Line 6018 in zombiemode.gsc, will kill this function!!
